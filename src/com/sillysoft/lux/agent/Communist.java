...
public class Communist extends SmartAgentBase {
    ...
    // We pick expando as the country we own that has the weakest enemy country beside it.
            ...
            if (countries[i].getOwner() == ID) {
                ...
                for (int j = 0; j < neighbors.length; j++) {
                    if (neighbors[j].getOwner() != ID && neighbors[j].getArmies() < leastNeighborArmies) {
                        if (!board.getAgentName(neighbors[j].getOwner()).equals(name()) || communismWins())
                        // don't attack other commies, until all the running dogs are dead
                        {
    ...
    protected boolean communismWins() {
        for (int i = 0; i < board.getNumberOfPlayers(); i++) {
            if (!board.getAgentName(i).equals(name()) && BoardHelper.playerIsStillInTheGame(i, countries)) {
...
// Hopefully this will propigate through to totally even out the armies in the long run.
        ...
        while (changed) {
            ...
            for (int i = 0; i < board.getNumberOfCountries(); i++) {
                if (countries[i].getOwner() == ID && countries[i].getMoveableArmies() > 0) {
                    // This means we COULD fortify out of this country if we wanted to.

                    // Get country[i]'s neighbors:
                    Country[] neighbors = countries[i].getAdjoiningList();

                    // Now loop through the neighbors and see if we own any of them.
                    for (int j = 0; j < neighbors.length && countries[i].getMoveableArmies() > 0; j++) {
                        if (neighbors[j].getOwner() == ID) {
                            ...
                            // So we own a neighbor. Let's see if they have more than one army difference:
                            if (difference > 1) {
