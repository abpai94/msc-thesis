...
public abstract class SmartAgentBase implements LuxAgent 
{
// This agent's ownerCode:
protected int ID;

// We store some refs to commonly used board objects
protected Board board;
protected Country[] countries;
protected int numCountries;
protected int numContinents;

// Sometimes in the attack phase we know if we want to move in armies or not.
// Store the value in this var. It will be used as long as it is not equal to -1.
protected int moveInMemory;

// Sometimes it is useful to remember which continent we are spending our efforts on.
...
// SmartAgentBase leaves it up to subclasses to implement the following methods:
public abstract String name();
public abstract int pickCountry();
public abstract void placeArmies( int numberOfArmies );
public abstract void attackPhase();
public abstract int moveArmiesIn( int cca, int ccd);
public abstract void fortifyPhase();
public abstract String youWon();



// I have yet to develope any card-smarts. The game-world will automatically cash in our best set if we return from this method and still have five or more cards.
...
// This method sets the class-variable goalCont to the continent with the least number of borders.
// First all totally non-occupied conts will be searched, then partially ocupied ones.
...
protected void setGoalToLeastBordersCont()
	...
	for (int i = 0; i < numContinents; i++)
		...
		if (board.getContinentBonus(i) > 0)
			...
			if (borderSizes[i] < smallBorders && BoardHelper.playerOwnsContinent(-1, i, countries))
				{
	...
	if (goalCont == -1)
		...
		for (int i = 0; i < numContinents; i++)
			...
			if (board.getContinentBonus(i) > 0)
				{
				if (borderSizes[i] < smallBorders && BoardHelper.playerOwnsContinentCountry(-1, i, countries))
					{
					smallBorders = borderSizes[i];
					goalCont = i;
					}
...
// If goalCont is set then return a country-code of the country we should choose in that cont. If goalCont is unset then set it to the smallest empty/open cont.
...
protected int pickCountryTouchingUs()
	...
	while (ci.hasNext())
		...
		if (open.getNumberPlayerNeighbors(ID) > maxTouches && board.getContinentBonus(open.getContinent()) >= 0)
			{
	...
	if (maxTouches < 1)
		...
		return BoardHelper.closestCountryWithOwner(ourBorderCountries, -1, countries);		
		}

	return maxCode;
	}

// returns the country-code of the nearest unowned country to the cluster starting at <root>
// NOTE, this method is inferior to pickCountryTouchingUs().
protected int pickCountryNearCluster( Country root )
	{
	// do a breadth first search outwards starting with this cluster's borders
	...
	for (int i = 0; i < cluster.size(); i++)
		...
		while ( neighbors.hasNext())
			...
			if ( neighbor.getOwner() == -1 )
				...
				{
				// Then we add it to the List. in time its neighbors will get expanded
...
protected boolean weOwnContsArround(Country center)
	...
	while (n.hasNext())
		{
		Country neib = n.next();
		if (neib.getContinent() != cont && ! BoardHelper.playerOwnsContinent(ID, neib.getContinent(), countries))
			{
			return false;
			}
		}
...
protected void
placeArmiesToTakeCont( int numberOfArmies, int wantCont )
	{
	if (wantCont == -1)
		{ // we weren't given a continent index. maybe there is only 1 continent. fallback to another method
		placeArmiesOnClusterBorder( numberOfArmies, BoardHelper.getPlayersBiggestArmy(ID, countries) );
		return;
		}
	...
	if (BoardHelper.playerOwnsContinent( ID, wantCont, countries))
		{
		// then we already own it, place on the weakest borders that we don't envelope
		...
		while ( placed < numberOfArmies )
			...
			for (int i = 0; i < borders.length; i++)
				{
				if (countries[borders[i]].getArmies() < leastArmies && ! weOwnContsArround(countries[borders[i]]))
					{
			...
			if (leastID == -1)
				{	// this can happen when the entire map is one continent. thus it has no borders
	...
	// if we own any countries in <wantCont>, then place the armies on the one with the most enemies inside the continent.
	...
	while (want.hasNext())
		...
		if (us.getOwner() == ID)
			...
			while (neighbors.hasNext())
				{
				Country neighbor = neighbors.next();
				if (neighbor.getOwner() != ID && neighbor.getContinent() == wantCont)
					enemyNeighbors++;
				}
	...
	if (bestPlace != null)
		{
		board.placeArmies(numberOfArmies, bestPlace);
		return;
		}

	// If we got here then we own zero countries inside <wantCont>
	// we place our armies in the country we own with the cheapest route to <wantCont>
	int[] route = BoardHelper.cheapestRouteFromOwnerToCont( ID, wantCont, countries );
	debug("BoardHelper.cheapestRouteFromOwnerToCont("+ID+", "+wantCont+") = "+new CountryRoute(route, countries));
	int placer = route[0];
	board.placeArmies(numberOfArmies, placer);
	}

// this method places armies one at a time on the weakest border surrounding <root>
...
protected void placeArmiesOnClusterBorder( int numberOfArmies, Country root )
	...
	if (weakest == null)
		...
		board.placeArmies( numberOfArmies, root );
		return;
		}
		
	int numberToPlace = Math.min(numberOfArmies, Math.max(1, numberOfArmies/100));
	
	board.placeArmies( numberToPlace, weakest );
...
protected boolean attackFillOut(Country root )
	...
	while (borders.hasNext()) {
		...
		while (neighbors.hasNext()) {
			Country neighbor = neighbors.next();
			if (neighbor.getOwner() != ID && neighbor.getNumberNotPlayerNeighbors(ID) == 0) {
				// attack it
	...
	while (borders.hasNext())
		...
		if (enemies == 1 && enemy.getNumberPlayerNeighbors(ID) > 1)
			...
			while (possibles.hasNext() )
				...
				if (poss.getOwner()==ID && poss.getNumberEnemyNeighbors()==1)
					{
			...
			if (ourArmies > enemy.getArmies())
				...
				for (int i = 0; i < ours.size() && enemy.getOwner() != ID; i++)
					{
					if (((Country)ours.get(i)).getArmies() > 1 && ((Country)ours.get(i)).canGoto(enemy))
						{
						moveInMemory = 1000000;
...
protected boolean attackSplitOff( Country root )
	...
	while (borders.hasNext()) 
		...
		while (neighbors.hasNext()) 
			{
			Country neighbor = neighbors.next();
			if (neighbor.getOwner() != ID && neighbor.getArmies() < border.getArmies()) 
				{
	...
	while (borders.hasNext()) {
		...
		if (border.getArmies() > enemiesArmies*attackRatio) {
			...
			while (neighbors.hasNext() && border.getArmies() > 1) {
				Country neighbor = neighbors.next();
				if (neighbor.getOwner() != ID) { // then we kill this enemy with 1/<enemies>
...
protected boolean tripleAttackPack(Country root)
	{
	boolean won = false;
	while ( attackEasyExpand(root) )	{won = true;}
	attackFillOut(root);
	while ( attackConsolidate(root) )	{won = true;}
	return won;
	}
...
protected void attackAsMuchAsPossible()
	...
	while (attacked) {
		...
		while (e.hasNext())
			{
			Country c = e.next();
			while ( tripleAttackPack(c) ) { attacked = true; }
			while ( attackSplitUp(c, 0.01f) ) { attacked = true; }
			}
		}
	}

// This method first checks to see if we can still aquire a card by conquering a country.
// If so, it compares all the possible attack match-ups we have and executes the best one 
// if (ourArmies > theirArmies*ratio). i.e. ratio of 1 is us > them. ratio of 2 is us double them
...
protected void attackForCard(int outnumberTimes)
	{
	if (! board.useCards() || board.tookOverACountry())
		{
		return;
		}
	...
	while (ours.hasNext())
		...
		while (thems.hasNext())
			...
			if (them.getOwner() != ID && (us.getArmies()*1.0/them.getArmies()) > bestRatio && us.canGoto(them))
				{
	...
	if ( bestUs != null && bestUs.getArmies() > bestThem.getArmies()*outnumberTimes )
		...
		setmoveInMemoryBeforeCardAttack(bestUs);		// boscoe and Yakool implement this differently
		board.attack(bestUs, bestThem, true);
		moveInMemory = -1;
		}
	}	// end of  attackForCard()


// to have rational behaviour when using attackForCard subclasses must imp this
protected void setmoveInMemoryBeforeCardAttack(Country attacker)
	{
	}


/**	If either country only borders 1 country then stay away from that country.
Return -1 if both countries have more then 1 neighbor, otherwise return the number that should be returned from moveArmiesIn(). */
...
/**	Check to see if moveInMemory has been set and if so obey it.
Return -1 if not set, otherwise return the number that should be returned from moveArmiesIn(). */
	...
	for (int i = 0; i < cluster.size(); i++) {
		...
		while ( neighbors.hasNext()) {
			...
			if ( neighbor.getOwner() == ID && ! cluster.contains(neighbor)) {
				...
				if (neighbor.canGoto( (Country)cluster.get(i))) { // this if statement should only return false when their are single-way borders on the map
...
/** Returns the contcode of the continent that we own with the most countries in it. */
protected int getBiggestOwnedCont()
	{
	int bestCont = -1;
	int bestContSize = -1;
	for(int i = 0; i < numContinents; i++)
		if (BoardHelper.playerOwnsContinent(ID, i, countries) && BoardHelper.getContinentSize(i,countries) > bestContSize)
			{
			bestCont = i;
...
/** Returns the contcode of the continent that we own with the largest positive bonus. */
protected int getMostValuablePositiveOwnedCont()
	{
	int bestCont = -1;
	int bestContBonus = -1;
	for(int i = 0; i < numContinents; i++)
		if (BoardHelper.playerOwnsContinent(ID, i, countries) && board.getContinentBonus(i) > bestContBonus)
			{
			bestCont = i;
...
// These methods used to be part of Yakool, but they were moved here so EvilPixie could take advantage of them
...
protected boolean placeArmiesToKillDominantPlayer(int numberOfArmies)
	...
	for (int i = 0; i < numPlayers; i++)
		{
		armies[i] = BoardHelper.getPlayerArmies(i, countries);
		incomes[i] = board.getPlayerIncome(i);
	...
	for (int i = 0; i < numPlayers; i++)
		{
		if (i != ID && (armies[i] >= totalArmies*0.5 || incomes[i] >= totalIncome*0.5 || ownedCountries[i] >= numCountries*0.5))
			playerToAttack = i;
		}
...
protected boolean placeArmiesToKillPlayer(int numberOfArmies, int playerToAttack)
	...
	mustKillPlayerOwnsCont = new boolean[numContinents];
	for (int i = 0; i < numContinents; i++)
		{
	...
	int[] cost = new int[numContinents];
	...
	for (int i = 0; i < numContinents; i++)
		...
		if (mustKillPlayerOwnsCont[i])
			{
			int[] path = BoardHelper.cheapestRouteFromOwnerToCont(ID, i, countries);
			if (path != null)
				{
...
debug("starting attackToKillPlayer -> "+player);
	...
	int[] ownContValue = new int[numContinents];
	for (int i = 0; i < numContinents; i++)
		{
		if (mustKillPlayerOwnsCont[i])
			ownContValue[i] = board.getContinentBonus(i);
		else
			ownContValue[i] = -1;
		}
	...
	while (biggestCont != -1)
		...
		if (biggestCont != -1)
			...
			ownContValue[biggestCont] = -1;		// so we don't try and attack it again
			}
		}
...
debug("starting attackToKillContinent -> "+cont);
	...
	while (armies.hasNext())
		...
		int[] path = BoardHelper.easyCostFromCountryToContinent(us.getCode(), cont, countries);
...
protected int pathCost( int[] path )
	...
	if (path==null)
		...
		return 1000000;		// return a high number so Yakool doesn't try to follow it
		}
	int cost = 0;
...
protected boolean attackAlongPath( int[] path )
	...
	for (int i = 1; i < path.length; i++)
		{
		moveInMemory = 1000000;
		if (countries[path[i-1]].getArmies() == 1 || board.attack(path[i-1], path[i], true) == 13)
			{
			// then we lost
			moveInMemory = -1;
			return false;
