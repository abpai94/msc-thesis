...
public class BetterPixie extends SmartAgentBase {
    float outnumberBy = 1;    // for individual country attacks
    protected int borderForce = 20;
    boolean[] ourConts;    // whether we will spend efforts taking/holding each continent
    ...
    boolean setupOurConts(int numberOfArmies) {
        ...
        for (int i = 0; i < numContinents; i++) {
            ...
            int ourArmiesNearCont = BoardHelper.getPlayerArmiesInContinent(ID, i, countries) +
                    BoardHelper.getPlayerArmiesAdjoiningContinent(ID, i, countries);

            // If we have a big group near the continent then consider that as well
            CountryRoute bestRoute = new CountryRoute(BoardHelper.cheapestRouteFromOwnerToCont(ID, i, countries), countries);
            int ourArmiesFartherAway = bestRoute.start().getArmies()
                    - (int) (bestRoute.costNotCountingPlayer(ID) * 1.2);

            neededForCont[i] -= Math.max(ourArmiesNearCont, ourArmiesFartherAway);
        }

        // We will only concentrate on the easiest continent to take when we own none
        boolean ownNoContinents = !BoardHelper.playerOwnsAnyPositiveContinent(ID, countries, board);
        ...
        for (int i = 0; i < numContinents; i++) {
            if (ownNoContinents) {
                if (neededForCont[i] < lowestArmiesNeededToTake && board.getContinentBonus(i) > 0) {
                    lowestArmiesNeededToTake = neededForCont[i];
                    targetCont = i;
                }
                ourConts[i] = false;
            }
            // say we can give at most numberOfArmies/(numContinents/4) armies to each continent.
            else if (neededForCont[i] < numberOfArmies / (numContinents / 4.0) && board.getContinentBonus(i) > 0) {
                ourConts[i] = true;
                wantACont = true;
            } else
                ourConts[i] = false;
        ...
        while (armiesPlaced < numberOfArmies && oneNeedsHelp) {
            ...
            for (int c = 0; c < numContinents; c++) {
                if (ourConts[c] && continentNeedsHelp(c)) {
                    debug("Placing an army to take continent " + board.getContinentName(c));
    ...
    /**
     * Place our armies so they are next to enemy clusters. If 'minimumToWin' is true then place the minimum number of armies needed to conquer each cluster (starting at the smallest). If 'minimumToWin' is false then place armies evenly for each cluster.
     */
    protected void placeNearEnemies(int numberOfArmies, boolean minimumToWin) {
        // Divide all enemy countries into clusters:
        List clustersWeBorder = new ArrayList();
        ...
        for (int i = 0; i < clusters.size() && numberOfArmies > 0; i++) {
            ...
            if (placeOn != null) {
                ...
                board.placeArmies(numberToPlace, placeOn);
    ...
    protected void attackInContinent(int cont) {
        ...
        int enemyCount = BoardHelper.getEnemyArmiesInContinent(ID, cont, countries);    // enemies in the cont
        ...
        // Count the friendlies (our armies in the cont and our armies in countries neighboring the cont)
        int friendlyCount = BoardHelper.getPlayerArmiesInContinent(ID, cont, countries);
        friendlyCount += BoardHelper.getPlayerArmiesAdjoiningContinent(ID, cont, countries);

        debug("Attacking in continent " + board.getContinentName(cont) + ". friendlies = " + friendlyCount + ", enemies = " + enemyCount);
        ...
        while (attackMade) {
            ...
            // Start by only attacking from our countries that have 1 enemy country
            while (attackMade) {
                attackMade = false;
                ...
                while (continent.hasNext()) {
                    ...
                    if (c.getOwner() == ID && getNumberOfEnemyNeighborsInOurConts(c) == 1 && c.getArmies() > 1) {
                        // This country can only attack 1 good destination, so attack it
                        Country[] adjoining = c.getAdjoiningList();
                        for (int i = 0; i < adjoining.length; i++)
                            if (adjoining[i].getOwner() != ID && ourConts[adjoining[i].getContinent()]
                                    && c.getArmies() > adjoining[i].getArmies() * outnumberBy) {
                                board.attack(c, adjoining[i], true);
                                attackMade = true;
                            }
                    }
                }
    ...
    protected void takeOutContinentCheck(int cont) {
        if (BoardHelper.anyPlayerOwnsContinent(cont, countries) && board.getContinentBonus(cont) > 0) {
...
/*
            int continentBonus = board.getContinentBonus(cont);
			int[] path = BoardHelper.cheapestRouteFromOwnerToCont(ID, cont, countries);
			if (path != null)
				{
				int costToHit = pathCost(path);
				if (costToHit < continentBonus)
					{
					System.out.println(board.getPlayerName(ID)+" thinks it is worth it to take out continent "+board.getContinentName(cont)+" from player "+board.getPlayerName(countries[BoardHelper.getCountryInContinent(cont, countries)].getOwner())+" ("+continentBonus+" bonus vs "+costToHit+" costToHit)");
					}
				}
*/
                ...
                for (int b = 0; b < borders.length; b++) {
                    ...
                    for (int n = 0; n < neigbors.length; n++) {
                        if (neigbors[n].getOwner() == ID &&
                                neigbors[n].getArmies() > countries[borders[b]].getArmies() * 2 && neigbors[n].canGoto(countries[borders[b]])) {
                            // kill him
    ...
    public int moveArmiesIn(int cca, int ccd) {
        ...
        // OK, so they both have some enemies. Look again only considering the enemies that are in conts we care about.
        // (And make a note of the enemies for later)
        List attackerEnemyList = new ArrayList(), defenderEnemyList = new ArrayList();
        attackerEnemies = 0;
        defenderEnemies = 0;
        ...
        // OK, so they both have some enemies in continents we care about. Do they connect?
        List allEnemies = new ArrayList(attackerEnemyList);
        ...
        for (int i = 0; i < numContinents; i++) {
            if (BoardHelper.playerOwnsContinent(ID, i, countries)) {
                while (fortifyContinent(i)) {
                    debug("Fortifying continent " + board.getContinentName(i));
                }
            } else {
        ...
        for (int i = 0; i < cluster.size(); i++) {
            ...
            while (neighbors.hasNext()) {
                ...
                if (neighbor.getOwner() == ID && !cluster.contains(neighbor)
                        && neighbor.getContinent() == cont && neighbor.getMoveableArmies() > 0) {
                    debug(" -> fortify " + neighbor.getMoveableArmies() + " armies from " + neighbor + " to " + cluster.get(i));
                    // Then <neighbor> is part of the cluster. fortify any armies back and add to the List
                    if (board.fortifyArmies(neighbor.getMoveableArmies(), neighbor, (Country) cluster.get(i)) == 1)
