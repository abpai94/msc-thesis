...
public class CastleInfinity implements LuxMapGenerator, ImageObserver {
  ...
  private void loadSIForeground() {
    try {
      ...
      String siForegroundPath = support.getPath() + File.separator + "MapGenerators" + File.separator + "CastleInfinityRes.png";
  ...
  public boolean generate(PrintWriter pw, String theChoice, int theSeed, MapLoader loader) {
    ...
    if(choice.equals(CHOICE_SMALL)) {
      ...
      height = 485;
      numCastles = 4;
    } else if(choice.equals(CHOICE_NORMAL)) {
      width = 785;
      height = 625;
      numCastles = 6;
    } else if(choice.equals(CHOICE_BIG)) {
      width = 1062;
      height = 765;
      numCastles = 10;
    } else if(choice.equals(CHOICE_FULL)) {
      ...
      height = (int) screenDimension.getHeight()-40-20;
      numCastles = width/100;
    } else
        {
        System.out.println("CastleInfinity unknown setting: "+choice);
        }
    ...
    backG.fillRect(0, 0, width, height);
      ...
      File support = new File(loader.getMapGeneratorPath()).getParentFile();
      String bgpath = support.getPath() + File.separator + "MapGenerators" + File.separator + "CastleInfinityBackground.jpg";

    File texture = new File(bgpath);
	if (texture.exists())
		{
        try {
            BufferedImage TextureImage = ImageIO.read(texture);
            backG.drawImage(TextureImage, 0, 0, width, height, this);
            } catch (Exception e) { e.printStackTrace(); }
		}
    else
        {
        System.out.println("CastleInfinity could not find background texture: "+bgpath);
        }
    ...
    out.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
              ...
              "<armySet>Medieval</armySet>\n" +
              "<shortContinentLabels>true</shortContinentLabels>\n" +
...
Rectangle playerInfoRect;
  ...
  private void makeHexagonalGrid() {
     ...
     if (choice.equals(CHOICE_SMALL))
        {
        // no room for PI inside the map. put it off to the side
        playerInfoRect = new Rectangle(0, 0, 0, 0);
        out.write("<playerInfoLocation>200,"+(height-115)+"</playerInfoLocation>\n");
        }
    else if (! m.isPlayerInfoInsideMap())
        {
        // they have settings set to not have PI inside the map
        playerInfoRect = new Rectangle(0, 0, 0, 0);
        out.write("<playerInfoLocation>0,"+(height-115)+"</playerInfoLocation>\n");
        }
    else
        {
        // normal case, make room inside the map for PI
        playerInfoRect = new Rectangle(0, 0, 290, 140);
        out.write("<playerInfoLocation>0,"+(height-115)+"</playerInfoLocation>\n");
        }
    ...
    for(double x = hexRad; x <= width-hexRad; x += Math.sqrt(3)*hexRad) {
      for(double y = hexRad-hexRad/3; y <= height-hexRad+hexRad/3; y += hexRad*3) {
        if(playerInfoRect.contains(new Point((int)x, (int)y)))
            {}
        else
  ...
  static final int cIR = 20; //castleInRadius, used to be 30
  ...
  private void writeHexagons() {
	...
	"  <continentname>" + countrysideNames[c] + "</continentname>\n" + 
      ...
      for(int i = 0; i < countryPolygons.size(); i += 1) { //write out each country
        ...
        out.write("<initialOwner>"+(c % getScenarioPlayerCountForSize(choice))+"</initialOwner>\n" +
		"<initialArmies>1</initialArmies>");
  ...
  private void writeBridges() {
	...
	"  <bonus>" + -2 + "</bonus>\n");
      ...
      for(int j = 0; j < 2; j += 1) { //two halves per bridge
        ...
        out.write("<initialOwner>"+((i+j) % getScenarioPlayerCountForSize(choice))+"</initialOwner>\n" +
		"<initialArmies>2</initialArmies>");
  ...
  private void doTheme() {
    ...
    try {
      ...
      new File(themes.getPath() + File.separator + choice + "_" + seed).mkdir();
      
      ImageIO.write(background, "png", new File(themes + File.separator + choice+ "_" + seed + File.separator + "background.png"));
      ImageIO.write(foreground, "png", new File(themes + File.separator + choice+ "_" + seed + File.separator + "overground.png"));
    } catch(Exception e) {
      debug(e.toString());
    }
  ...
  private static final Point[] siCastlePoints = new Point[]{new Point(260, 100),
                                                            new Point(228, 607-44),
                                                            new Point(938, 109-44),
                                                            new Point(937, 218-44),
                                                            new Point(938, 302-44),
                                                            new Point(938, 385-44),
                                                            new Point(935, 477-44),
                                                            new Point(942, 561-44),
                                                            new Point(941, 644-44)};
  ...
  static final String CHOICE_FULL = "CastleInfinity - fullscreen";
...
public int getScenarioPlayerCountForSize(String boardSize)
	{
	if (boardSize.equals(CHOICE_SMALL))
		return 4;
	if (boardSize.equals(CHOICE_NORMAL))
		return 6;
	if (boardSize.equals(CHOICE_BIG))
		return 6;
	if (boardSize.equals(CHOICE_FULL))
		return 6;
...
"gary the cheater","shock-n-ya'll","stevedip","Grozoth","Mud","fink","Natya","mood in dhingo","fishflakes","Preacherman","mikey","Zoe","kevquincey","Baden","Chumdinger","Nietzsche",
...
"SexBomba","primaimm","mood in dhingo","Muskie","Mattman","cheech","Black Pope","matthiasmay","Kleddiator","Toddly","Vidar","what? oh yeah.",
...
// added more in 2015:
"upeng2005", "Baden", "Royalion", "shopi", "Pauley", "GODZILLA", "General K", "General Zod", "pcristov", "Cortes", "Soloman Grundy", "GFips", "Hugo", "I like cold beverages", "erdi", "Loki", "meron", "3DA", "monkeydog", "MASSHOLE", "Big Will E Style", "George", "Zman", "Killercatfish", "Engie", "funkyfreddy", "Big¢Daddy", "Mumps", "JSW", "Stanley Cup 2", "Darth Rellek", "obiwan", "Licht", "Guillerme", "glu", "yls", "Stelee", "snarl", "paranoiarodeo", "gypsymystyc", "magpie", "Behemoth", "Naraku", "R@wby", "LuxCampCoach", "PitytheFool", "cheech99", "Frank549", "gunz", "Mazza", "rubeng", "'84 Tigers", "djdee", "KingPatrick", "Marauder", "Sexy Lady", "saxiboysmith", "controleola", "mohegan", "ocpTV", "WhirlPlaid", "dollabillz", "Samuel99", "Dr.Thorvaldt", "GYPCOUNT", "Mr. Potato Head", "the Wizard of", "Smashley", "trinkett", "Gabo", "MrBuckin the Impaler", "vargas", "Hatszel", "Bod", "Dad", "Mushy Peas", "ricardo", "Dude-man", "Rich Girl", "AlphA_OmegA", "h0b0", "sergio el capo", "C-Rex", "Jimmy Walker", "mood in dhingo", "*Manimal", "Dominator", "compton nick", "K-SWISS", "Yo Daddy", "Packman", "Mnemosyne", "Mouldy Dog", "ccarson", "Toxik", "Rated Insane", "PJR", "Milltycoon", "Kilauea	1937", "newbeslaya", "", "", "Royalion", "upeng2005", "General K", "I like cold beverages", "Hugo", "Cortes", "erdi", "3DA", "Big Will E Style", "Darth Rellek", "meron", "George874", "Pauley", "paranoiarodeo", "Killercatfish", "magpie", "funkyfreddy", "cheech99", "Engie", "gunz", "Mumps", "Behemoth", "'84 Tigers", "pcristov", "gypsymystyc", "Smashley", "Baden", "Frank549", "saxiboysmith", "Zman", "Stelee", "Marauder", "Naraku", "rubeng", "snarl", "Gabo", "Bod", "dollabillz", "Dr.Thorvaldt", "C-Rex", "Rated Insane", "AlphA_OmegA", "Yo Daddy", "*Manimal", "R@wby", "The ROOSTER", "Mnemosyne", "DeputyDawg", "newbeslaya", "Mushy Peas", "djdee", "velvateen", "Packman", "WETSU", "Schroberts", "compton nick", "Soloman Grundy", "Hatszel", "ocpTV", "Samuel99", "trinkett", "Mazza", "supreme commander", "PitytheFool", "mood in dhingo", "Mr. Potato Head", "controleola", "kitty on catnip", "Identity", "tomcotree", "SecondTermMistake", "Mouldy Dog", "pan", "GFips", "the Wizard of", "Plane crazy", "Lantern", "CET", "BLACKDOG", "Darth Casey", "Dominator", "h0b0", "Milltycoon", "alessa", "GYPCOUNT", "Rich Girl", "lippet", "AquaRegia", "PSU7685", "Nietzsche", "Gnome", "yls", "Sexy Lady", "SantaClaus", "Dangerous Beans", "Guillerme", "animechess", "Yastemajo", "Heartscoffee", "ricardo"
    };
