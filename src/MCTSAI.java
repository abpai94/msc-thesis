import Support.BoardState;
import Support.MCTSHelper;
import Support.*;
 ...
 */
public class MCTSAI implements LuxAgent {

    public int ID;
    public Board board;
    public final int numberOfSimulations = 10000;
    ...
    public void setPrefs(int ID, Board board) {
        ...
        random = new Random();
        boardState = new BoardState(ID, board, logWriter, numberOfSimulations, random);
        placeArmies = new PlaceArmies(ID, board, logWriter);
        attackPhase = new AttackPhase(ID, board, logWriter);
        mctsHelper = boardState.mctsHelper;
        countries = boardState.getCountries();
    }
     ...
     * must return the country-code of an unowned country. (Unowned countries have ownerCode's of -1).
     * <p>
     * If preferences are set to pick the initial countries randomly, then this method will never be called.
     */
    @Override
    public int pickCountry() {
        mctsHelper.pickCountry();
        int bestChoice = -1;
        int bestChoiceSoFar = 0;
        countries = boardState.getCountries();
        for (Map.Entry<Integer, Integer> entry : mctsHelper.winningChoices.entrySet()) {
            if (entry.getValue() > bestChoiceSoFar && countries[entry.getKey()].getOwner() == -1) {
                bestChoiceSoFar = entry.getValue();
